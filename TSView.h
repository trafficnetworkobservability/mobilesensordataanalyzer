...
//output
	...
	void ConstructCumulativeFlowVector(float data_collection_time_interval_in_sec, int number_of_time_intervals)
	{
...
public:
	...
	//SITSLAB
	bool m_bExcludeLaneChangingVehicles;
...
//	void DrawTrain(int TrainNo);
	...
	void Output_NGSIM_Matrix(string output_type);
	void Output_NGSIM_Matrix(string output_type, float SensorSpacing, map<float, map<int, float>> & Distance_Time_Value);  //output Y-axis as distance instead of sensorID
	void Output_NGSIM_Individual_Vehicle_Trajectory(int display_resolution, CRect PlotRect, map<float, map<int, float>> & Distance_Time_Value);
