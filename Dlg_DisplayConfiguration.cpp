...
void CDlg_DisplayConfiguration::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST_GPS_CAR_TEXT_LABEL, m_VehicleIDList);
	DDX_Control(pDX, IDC_LIST_GPS_DAY_TEXT_LABEL2, m_PassengerIDList);
}
...
//	m_Movement_Label.AddString ("Observed Total Count");
	...
	m_PassengerIDList.AddString("All");

	for (int pax = 0; pax < pView->GetDocument ()->m_PassengerIDStringVector.size(); pax++)
	{
		m_PassengerIDList.AddString(pView->GetDocument()->m_PassengerIDStringVector[pax].c_str());
	}
	m_PassengerIDList.AddString("none");

	m_PassengerIDList.SetCurSel(0);
	CString GPSDayString;

	m_VehicleIDList.AddString("All");

	for (int car = 0; car < pView->GetDocument()->m_VehicleIDStringVector.size(); car++)
	{
		m_VehicleIDList.AddString(pView->GetDocument()->m_VehicleIDStringVector[car].c_str());
	}
	m_VehicleIDList.AddString("none");

	m_VehicleIDList.SetCurSel(0);
